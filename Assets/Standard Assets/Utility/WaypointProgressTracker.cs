...
namespace UnityStandardAssets.Utility
{
    public class WaypointProgressTracker : MonoBehaviour
    {
        ...
        public void Reset()
        {
            ...
            {
                target.position = circuit.Waypoints[_progressNum].position;
                target.rotation = circuit.Waypoints[_progressNum].rotation;
            }
        }
        ...
        private void Update()
        {
            if (progressStyle == ProgressStyle.SmoothAlongRoute)
            {
                ...
                target.position =
                    circuit.GetRoutePoint(_progressDistance + lookAheadForTargetOffset + lookAheadForTargetFactor*_speed)
                           .Position;
                target.rotation =
                    Quaternion.LookRotation(
                        circuit.GetRoutePoint(_progressDistance + lookAheadForSpeedOffset + lookAheadForSpeedFactor*_speed)
                               .Direction);
        ...
        private void OnDrawGizmos()
        {
            if (Application.isPlaying)
            {
                ...
                Gizmos.DrawWireSphere(circuit.GetRoutePosition(_progressDistance), 1);

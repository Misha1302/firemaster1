...
namespace UnityStandardAssets.Water
{
    ...
    public class Water : MonoBehaviour
    {
        ...
        private Dictionary<Camera, Camera> _mReflectionCameras = new Dictionary<Camera, Camera>(); // Camera -> Camera table
        private Dictionary<Camera, Camera> _mRefractionCameras = new Dictionary<Camera, Camera>(); // Camera -> Camera table
        ...
        void CreateWaterObjects(Camera currentCamera, out Camera reflectionCamera, out Camera refractionCamera)
        {
            ...
            if (mode >= WaterMode.Reflective)
            {
                ...
                if (!_mReflectionTexture || _mOldReflectionTextureSize != textureSize)
                {
                    ...
                    _mReflectionTexture = new RenderTexture(textureSize, textureSize, 16);
                    _mReflectionTexture.name = "__WaterReflection" + GetInstanceID();
            ...
            if (mode >= WaterMode.Refractive)
            {
                ...
                if (!_mRefractionTexture || _mOldRefractionTextureSize != textureSize)
                {
                    ...
                    _mRefractionTexture = new RenderTexture(textureSize, textureSize, 16);
                    _mRefractionTexture.name = "__WaterRefraction" + GetInstanceID();
                    _mRefractionTexture.isPowerOfTwo = true;
                    _mRefractionTexture.hideFlags = HideFlags.DontSave;
        ...
        {
            if (_mHardwareWaterSupport < waterMode)
            {

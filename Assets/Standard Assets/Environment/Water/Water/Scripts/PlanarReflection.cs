...
namespace UnityStandardAssets.Water
{
    ...
    public class PlanarReflection : MonoBehaviour
    {
        ...
        Vector3 _mOldpos;
        Camera _mReflectionCamera;
        Material _mSharedMaterial;
        Dictionary<Camera, bool> _mHelperCameras;


        public void Start()
        {
            _mSharedMaterial = ((WaterBase)gameObject.GetComponent(typeof(WaterBase))).sharedMaterial;
        }
        ...
        {
            if (null == _mHelperCameras)
            {
                _mHelperCameras = new Dictionary<Camera, bool>();
            }

            if (!_mHelperCameras.ContainsKey(currentCam))
            {
                _mHelperCameras.Add(currentCam, false);
            }
            if (_mHelperCameras[currentCam])
            {
                return;
            }

            if (!_mReflectionCamera)
            {
                _mReflectionCamera = CreateReflectionCameraFor(currentCam);
            }

            RenderReflectionFor(currentCam, _mReflectionCamera);

            _mHelperCameras[currentCam] = true;
        }
        ...
        public void WaterTileBeingRendered(Transform tr, Camera currentCam)
        {
            ...
            if (_mReflectionCamera && _mSharedMaterial)
            {
                _mSharedMaterial.SetTexture(reflectionSampler, _mReflectionCamera.targetTexture);
            }
        }
        ...
        void RenderReflectionFor(Camera cam, Camera reflectCamera)
        {
            ...
            if (_mSharedMaterial && !_mSharedMaterial.HasProperty(reflectionSampler))
            {
                return;
            }
            ...
            _mOldpos = cam.transform.position;
            Vector3 newpos = reflection.MultiplyPoint(_mOldpos);
